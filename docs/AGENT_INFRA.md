...
- **Ansible в этом репозитории — без отдельных Python-скриптов:** не добавлять файлы `*.py` под деплой (в т.ч. в `ansible/scripts/`) и не копировать такие скрипты на целевые хосты для шагов панели/Xray. Логику выносить в **`*.yml` задачи**, Jinja и модули Ansible (`uri`, `set_fact`, `template`, …). Инвентарь для CI уже собирается плейбуком **`ansible/ci/ci-write-inventory.yml`** без хелперов на Python.
